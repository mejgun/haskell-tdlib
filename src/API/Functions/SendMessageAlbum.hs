...
-- 
-- __only_preview__ Pass true to get fake messages instead of actually sending them
data SendMessageAlbum = 

 SendMessageAlbum { only_preview :: Maybe Bool, input_message_contents :: Maybe [InputMessageContent.InputMessageContent], options :: Maybe MessageSendOptions.MessageSendOptions, reply_to_message_id :: Maybe Int, message_thread_id :: Maybe Int, chat_id :: Maybe Int }  deriving (Eq)
...
instance T.ToJSON SendMessageAlbum where
 toJSON SendMessageAlbum { only_preview = only_preview, input_message_contents = input_message_contents, options = options, reply_to_message_id = reply_to_message_id, message_thread_id = message_thread_id, chat_id = chat_id } =
  A.object [ "@type" A..= T.String "sendMessageAlbum", "only_preview" A..= only_preview, "input_message_contents" A..= input_message_contents, "options" A..= options, "reply_to_message_id" A..= reply_to_message_id, "message_thread_id" A..= message_thread_id, "chat_id" A..= chat_id ]
 ...
 parseJSON v@(T.Object obj) = do
  ...
  where
   ...
   parseSendMessageAlbum = A.withObject "SendMessageAlbum" $ \o -> do
    ...
    return $ SendMessageAlbum { only_preview = only_preview, input_message_contents = input_message_contents, options = options, reply_to_message_id = reply_to_message_id, message_thread_id = message_thread_id, chat_id = chat_id }
 parseJSON _ = mempty
