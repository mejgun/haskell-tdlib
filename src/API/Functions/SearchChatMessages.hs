...
-- 
-- __sender__ Sender of messages to search for; pass null to search for messages from any sender. Not supported in secret chats
-- 
...
data SearchChatMessages = 
 ...
 SearchChatMessages { message_thread_id :: Maybe Int, _filter :: Maybe SearchMessagesFilter.SearchMessagesFilter, limit :: Maybe Int, offset :: Maybe Int, from_message_id :: Maybe Int, sender :: Maybe MessageSender.MessageSender, query :: Maybe String, chat_id :: Maybe Int }  deriving (Eq)

instance Show SearchChatMessages where
 show SearchChatMessages { message_thread_id=message_thread_id, _filter=_filter, limit=limit, offset=offset, from_message_id=from_message_id, sender=sender, query=query, chat_id=chat_id } =
  "SearchChatMessages" ++ cc [p "message_thread_id" message_thread_id, p "_filter" _filter, p "limit" limit, p "offset" offset, p "from_message_id" from_message_id, p "sender" sender, p "query" query, p "chat_id" chat_id ]
