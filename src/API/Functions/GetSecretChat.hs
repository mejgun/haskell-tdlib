...
data GetSecretChat = 
 ...
 GetSecretChat { secret_chat_id :: Maybe Int }  deriving (Eq)

instance Show GetSecretChat where
 show GetSecretChat { secret_chat_id=secret_chat_id } =
  "GetSecretChat" ++ U.cc [U.p "secret_chat_id" secret_chat_id ]
