...
-- 
-- __is_hand_raised__ Pass true if the user's hand should be raised. Only self hand can be raised. Requires groupCall.can_be_managed group call flag to lower other's hand
data ToggleGroupCallParticipantIsHandRaised = 

 ToggleGroupCallParticipantIsHandRaised { is_hand_raised :: Maybe Bool, participant_id :: Maybe MessageSender.MessageSender, group_call_id :: Maybe Int }  deriving (Show, Eq)
