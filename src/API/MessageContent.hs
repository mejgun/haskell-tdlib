...
data MessageContent = 
 ...
 MessageVoiceChatScheduled { start_date :: Maybe Int, group_call_id :: Maybe Int }  |
 -- |
 -- 
 -- A newly created voice chat 
 -- 
 -- __group_call_id__ Identifier of the voice chat. The voice chat can be received through the method getGroupCall
 MessageVoiceChatStarted { group_call_id :: Maybe Int }  |
 -- |
 -- 
 -- A message with information about an ended voice chat 
 -- 
 -- __duration__ Call duration, in seconds
 MessageVoiceChatEnded { duration :: Maybe Int }  |
 -- |
 -- 
 ...
 MessageInviteVoiceChatParticipants { user_ids :: Maybe [Int], group_call_id :: Maybe Int }  |
 -- |
 -- 
 ...
 -- 
 -- __ttl__ New message TTL setting
 ...
 -- 
 -- __traveler__ The user or chat, which triggered the proximity alert
 -- 
 -- __watcher__ The user or chat, which subscribed for the proximity alert
 -- 
 -- __distance__ The distance between the users
 MessageProximityAlertTriggered { distance :: Maybe Int, watcher :: Maybe MessageSender.MessageSender, traveler :: Maybe MessageSender.MessageSender }  |
 -- |
 -- 
...
instance T.FromJSON MessageContent where
 parseJSON v@(T.Object obj) = do
  ...
  case t of
   ...
   "messageVoiceChatScheduled" -> parseMessageVoiceChatScheduled v
   "messageVoiceChatStarted" -> parseMessageVoiceChatStarted v
   "messageVoiceChatEnded" -> parseMessageVoiceChatEnded v
   "messageInviteVoiceChatParticipants" -> parseMessageInviteVoiceChatParticipants v
  ...
  where
   ...
   parseMessageVoiceChatScheduled :: A.Value -> T.Parser MessageContent
   parseMessageVoiceChatScheduled = A.withObject "MessageVoiceChatScheduled" $ \o -> do
    ...
    return $ MessageVoiceChatScheduled { start_date = start_date, group_call_id = group_call_id }

   parseMessageVoiceChatStarted :: A.Value -> T.Parser MessageContent
   parseMessageVoiceChatStarted = A.withObject "MessageVoiceChatStarted" $ \o -> do
    group_call_id <- mconcat [ o A..:? "group_call_id", readMaybe <$> (o A..: "group_call_id" :: T.Parser String)] :: T.Parser (Maybe Int)
    return $ MessageVoiceChatStarted { group_call_id = group_call_id }

   parseMessageVoiceChatEnded :: A.Value -> T.Parser MessageContent
   parseMessageVoiceChatEnded = A.withObject "MessageVoiceChatEnded" $ \o -> do
    duration <- mconcat [ o A..:? "duration", readMaybe <$> (o A..: "duration" :: T.Parser String)] :: T.Parser (Maybe Int)
    return $ MessageVoiceChatEnded { duration = duration }

   parseMessageInviteVoiceChatParticipants :: A.Value -> T.Parser MessageContent
   parseMessageInviteVoiceChatParticipants = A.withObject "MessageInviteVoiceChatParticipants" $ \o -> do
    user_ids <- o A..:? "user_ids"
    group_call_id <- mconcat [ o A..:? "group_call_id", readMaybe <$> (o A..: "group_call_id" :: T.Parser String)] :: T.Parser (Maybe Int)
    return $ MessageInviteVoiceChatParticipants { user_ids = user_ids, group_call_id = group_call_id }
   ...
   parseMessageProximityAlertTriggered = A.withObject "MessageProximityAlertTriggered" $ \o -> do
    ...
    watcher <- o A..:? "watcher"
    traveler <- o A..:? "traveler"
    return $ MessageProximityAlertTriggered { distance = distance, watcher = watcher, traveler = traveler }
