...
data GetPremiumGiftCodePaymentOptions
  = GetPremiumGiftCodePaymentOptions
    { boosted_chat_id :: Maybe Int -- ^ Identifier of the channel chat, which will be automatically boosted by receivers of the gift codes and which is administered by the user; 0 if none
    }
  deriving (Eq, Show)
