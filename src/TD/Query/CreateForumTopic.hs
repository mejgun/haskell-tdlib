...
-- | Creates a topic in a forum supergroup chat or a chat with a bot with topics; requires can_manage_topics administrator or can_create_topics member right in the supergroup. Returns 'TD.Data.ForumTopicInfo.ForumTopicInfo'
data CreateForumTopic
  = CreateForumTopic
    { chat_id          :: Maybe Int                           -- ^ Identifier of the chat
    , name             :: Maybe T.Text                        -- ^ Name of the topic; 1-128 characters
    , is_name_implicit :: Maybe Bool                          -- ^ Pass true if the name of the topic wasn't entered explicitly; for chats with bots only
    ...
    CreateForumTopic
      ...
      , icon             = icon_
        ...
        = A.object
          ...
          , "chat_id"          A..= chat_id_
          , "name"             A..= name_
          , "is_name_implicit" A..= is_name_implicit_
          , "icon"             A..= icon_
          ]
