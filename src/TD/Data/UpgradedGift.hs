...
data UpgradedGift
  = UpgradedGift -- ^ Describes an upgraded gift that can be transferred to another owner or transferred to the TON blockchain as an NFT
    ...
    , is_burned                   :: Maybe Bool                                                    -- ^ True, if the gift was used to craft another gift
    , is_crafted                  :: Maybe Bool                                                    -- ^ True, if the gift was craft from another gifts
    ...
    , colors                      :: Maybe UpgradedGiftColors.UpgradedGiftColors                   -- ^ Colors that can be set for user's name, background of empty chat photo, replies to messages and link previews; may be null if none or unknown
    ...
    , can_send_purchase_offer     :: Maybe Bool                                                    -- ^ True, if an offer to purchase the gift can be sent using sendGiftPurchaseOffer
    , craft_probability_per_mille :: Maybe Int                                                     -- ^ Probability that the gift adds to the chance of successful crafting of a new gift; 0 if the gift can't be used for crafting
    ...
    , value_usd_amount            :: Maybe Int                                                     -- ^ Estimated value of the gift in USD; in USD cents; 0 if unavailable
    }
  deriving (Eq, Show)
...
instance I.ShortShow UpgradedGift where
  shortShow UpgradedGift
    ...
    , can_send_purchase_offer     = can_send_purchase_offer_
    , craft_probability_per_mille = craft_probability_per_mille_
    , value_currency              = value_currency_
    , value_amount                = value_amount_
    , value_usd_amount            = value_usd_amount_
    }
      = "UpgradedGift"
        ++ I.cc
        [ "_id"                         `I.p` _id_
        ...
        , "can_send_purchase_offer"     `I.p` can_send_purchase_offer_
        , "craft_probability_per_mille" `I.p` craft_probability_per_mille_
        , "value_currency"              `I.p` value_currency_
        , "value_amount"                `I.p` value_amount_
        , "value_usd_amount"            `I.p` value_usd_amount_
        ]
  ...
  parseJSON v@(AT.Object obj) = do
    ...
    where
      ...
      parseUpgradedGift = A.withObject "UpgradedGift" $ \o -> do
        ...
        can_send_purchase_offer_     <- o A..:?                       "can_send_purchase_offer"
        craft_probability_per_mille_ <- o A..:?                       "craft_probability_per_mille"
        value_currency_              <- o A..:?                       "value_currency"
        value_amount_                <- o A..:?                       "value_amount"
        value_usd_amount_            <- o A..:?                       "value_usd_amount"
        pure $ UpgradedGift
          { _id                         = _id_
          , regular_gift_id             = regular_gift_id_
          , publisher_chat_id           = publisher_chat_id_
