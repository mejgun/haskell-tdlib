...
data ForumTopics
  = ForumTopics -- ^ Describes a list of forum topics
    ...
    , next_offset_forum_topic_id :: Maybe Int                     -- ^ Offset forum topic identifier for the next getForumTopics request
    }
  deriving (Eq, Show)
...
instance I.ShortShow ForumTopics where
  shortShow ForumTopics
    ...
    , next_offset_forum_topic_id = next_offset_forum_topic_id_
    }
      = "ForumTopics"
        ++ I.cc
        ...
        , "next_offset_forum_topic_id" `I.p` next_offset_forum_topic_id_
        ]
  ...
  parseJSON v@(AT.Object obj) = do
    ...
    where
      ...
      parseForumTopics = A.withObject "ForumTopics" $ \o -> do
        ...
        next_offset_forum_topic_id_ <- o A..:?  "next_offset_forum_topic_id"
        pure $ ForumTopics
          { total_count                = total_count_
          , topics                     = topics_
          , next_offset_date           = next_offset_date_
