...
import {-# SOURCE #-} qualified TD.Data.Poll as Poll
...
data MessageContent
  ...
  | MessageDice -- ^ A dice message. The dice value is randomly generated by the server
    ...
    , value                          :: Maybe Int                       -- ^ The dice value. If the value is 0, then the dice don't have final state yet
  ...
  | MessagePoll -- ^ A message with a poll
    { poll           :: Maybe Poll.Poll                   -- ^ Information about the poll
    , description    :: Maybe FormattedText.FormattedText -- ^ Description of the poll
    , _media         :: Maybe MessageContent              -- ^ Media attached to the poll; may be null if none. If present, currently, can be only of the types messageAnimation, messageAudio, messageDocument, messageLocation, messagePhoto, messageVenue, or messageVideo without caption
    , can_add_option :: Maybe Bool                        -- ^ True, if an option can be added to the poll using addPollOption
    }
  | MessageStakeDice -- ^ A stake dice message. The dice value is randomly generated by the server
    { initial_state        :: Maybe DiceStickers.DiceStickers -- ^ The animated stickers with the initial dice animation; may be null if unknown. The update updateMessageContent will be sent when the sticker became known
    , final_state          :: Maybe DiceStickers.DiceStickers -- ^ The animated stickers with the final dice animation; may be null if unknown. The update updateMessageContent will be sent when the sticker became known
    , value                :: Maybe Int                       -- ^ The dice value. If the value is 0, then the dice don't have final state yet
    , stake_toncoin_amount :: Maybe Int                       -- ^ The Toncoin amount that was staked; in the smallest units of the currency
    , prize_toncoin_amount :: Maybe Int                       -- ^ The Toncoin amount that was gained from the roll; in the smallest units of the currency; -1 if the dice don't have final state yet
    }
  ...
  | MessagePollOptionAdded -- ^ A message with information about an added poll option
    { poll_message_id :: Maybe Int                         -- ^ Identifier of the message with the poll; can be an identifier of a deleted message or 0
    , option_id       :: Maybe T.Text                      -- ^ Identifier of the added option in the poll
    , text            :: Maybe FormattedText.FormattedText -- ^ Text of the option; 1-100 characters; may contain only custom emoji entities
    }
  | MessagePollOptionDeleted -- ^ A message with information about a deleted poll option
    { poll_message_id :: Maybe Int                         -- ^ Identifier of the message with the poll; can be an identifier of a deleted message or 0
    , option_id       :: Maybe T.Text                      -- ^ Identifier of the deleted option in the poll
    , text            :: Maybe FormattedText.FormattedText -- ^ Text of the option; 1-100 characters; may contain only custom emoji entities
    }
  ...
  | MessageManagedBotCreated -- ^ A bot managed by another bot was created by the user
    { bot_user_id :: Maybe Int -- ^ User identifier of the created bot
    }
  ...
  | MessageUpgradedGiftPurchaseOfferRejected -- ^ An offer to purchase a gift was rejected or expired
    { _gift            :: Maybe UpgradedGift.UpgradedGift       -- ^ The gift
    , price            :: Maybe GiftResalePrice.GiftResalePrice -- ^ The proposed price
    , offer_message_id :: Maybe Int                             -- ^ Identifier of the message with purchase offer which was rejected or expired; may be 0 or an identifier of a deleted message
    , was_expired      :: Maybe Bool                            -- ^ True, if the offer has expired; otherwise, the offer was explicitly rejected
    }
...
instance I.ShortShow MessageContent where
  ...
  shortShow MessagePoll
    ...
    , _media         = _media_
    , can_add_option = can_add_option_
    }
      = "MessagePoll"
        ++ I.cc
        [ "poll"           `I.p` poll_
        ...
        , "_media"         `I.p` _media_
        , "can_add_option" `I.p` can_add_option_
        ]
  shortShow MessageStakeDice
    { initial_state        = initial_state_
    , final_state          = final_state_
    , value                = value_
    , stake_toncoin_amount = stake_toncoin_amount_
    , prize_toncoin_amount = prize_toncoin_amount_
    }
      = "MessageStakeDice"
        ++ I.cc
        [ "initial_state"        `I.p` initial_state_
        ...
        , "prize_toncoin_amount" `I.p` prize_toncoin_amount_
        ]
  ...
  shortShow MessagePollOptionAdded
    { poll_message_id = poll_message_id_
    , option_id       = option_id_
    , text            = text_
    }
      = "MessagePollOptionAdded"
        ++ I.cc
        [ "poll_message_id" `I.p` poll_message_id_
        , "option_id"       `I.p` option_id_
        , "text"            `I.p` text_
        ]
  shortShow MessagePollOptionDeleted
    { poll_message_id = poll_message_id_
    , option_id       = option_id_
    , text            = text_
    }
  ...
  shortShow MessageManagedBotCreated
  ...
  shortShow MessageUpgradedGiftPurchaseOfferRejected
    { _gift            = _gift_
    , price            = price_
...
instance AT.FromJSON MessageContent where
  parseJSON v@(AT.Object obj) = do
    ...
    case t of
      ...
      "messagePollOptionAdded"                         -> parseMessagePollOptionAdded v
      "messagePollOptionDeleted"                       -> parseMessagePollOptionDeleted v
      ...
      "messageManagedBotCreated"                       -> parseMessageManagedBotCreated v
    ...
    where
      ...
      parseMessagePoll = A.withObject "MessagePoll" $ \o -> do
        ...
        _media_         <- o A..:?  "media"
        can_add_option_ <- o A..:?  "can_add_option"
        pure $ MessagePoll
          { poll           = poll_
          , description    = description_
          , _media         = _media_
      ...
      parseMessageStakeDice :: A.Value -> AT.Parser MessageContent
      parseMessageStakeDice = A.withObject "MessageStakeDice" $ \o -> do
        initial_state_        <- o A..:?  "initial_state"
        final_state_          <- o A..:?  "final_state"
        value_                <- o A..:?  "value"
        stake_toncoin_amount_ <- o A..:?  "stake_toncoin_amount"
        prize_toncoin_amount_ <- o A..:?  "prize_toncoin_amount"
        pure $ MessageStakeDice
          { initial_state        = initial_state_
          , final_state          = final_state_
          , value                = value_
          , stake_toncoin_amount = stake_toncoin_amount_
      ...
      parseMessagePollOptionAdded :: A.Value -> AT.Parser MessageContent
      parseMessagePollOptionAdded = A.withObject "MessagePollOptionAdded" $ \o -> do
        poll_message_id_ <- o A..:?  "poll_message_id"
        option_id_       <- o A..:?  "option_id"
        text_            <- o A..:?  "text"
        pure $ MessagePollOptionAdded
          { poll_message_id = poll_message_id_
          , option_id       = option_id_
          , text            = text_
          }
      parseMessagePollOptionDeleted :: A.Value -> AT.Parser MessageContent
      parseMessagePollOptionDeleted = A.withObject "MessagePollOptionDeleted" $ \o -> do
        poll_message_id_ <- o A..:?  "poll_message_id"
        option_id_       <- o A..:?  "option_id"
        text_            <- o A..:?  "text"
        pure $ MessagePollOptionDeleted
          { poll_message_id = poll_message_id_
          , option_id       = option_id_
          , text            = text_
          }
      ...
      parseMessageManagedBotCreated :: A.Value -> AT.Parser MessageContent
      parseMessageManagedBotCreated = A.withObject "MessageManagedBotCreated" $ \o -> do
        bot_user_id_ <- o A..:?  "bot_user_id"
        pure $ MessageManagedBotCreated
          { bot_user_id = bot_user_id_
          }
      ...
      parseMessageUpgradedGiftPurchaseOfferRejected :: A.Value -> AT.Parser MessageContent
      parseMessageUpgradedGiftPurchaseOfferRejected = A.withObject "MessageUpgradedGiftPurchaseOfferRejected" $ \o -> do
        _gift_            <- o A..:?  "gift"
        price_            <- o A..:?  "price"
        offer_message_id_ <- o A..:?  "offer_message_id"
        was_expired_      <- o A..:?  "was_expired"
        pure $ MessageUpgradedGiftPurchaseOfferRejected
          { _gift            = _gift_
          , price            = price_
          , offer_message_id = offer_message_id_
          , was_expired      = was_expired_
