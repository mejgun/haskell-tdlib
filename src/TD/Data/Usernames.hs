...
data Usernames
  = Usernames -- ^ Describes usernames assigned to a user, a supergroup, or a channel
    ...
    , editable_username  :: Maybe T.Text   -- ^ Active or disabled username, which may be changed with setUsername or setSupergroupUsername. Information about other active usernames can be received using getCollectibleItemInfo
    }
  deriving (Eq, Show)
