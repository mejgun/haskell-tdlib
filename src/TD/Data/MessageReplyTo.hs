...
data MessageReplyTo
  = MessageReplyToMessage -- ^ Describes a message replied by a given message
    ...
    , origin           :: Maybe MessageOrigin.MessageOrigin   -- ^ Information about origin of the message if the message was replied from another chat; may be null for messages from the same chat
    , origin_send_date :: Maybe Int                           -- ^ Point in time (Unix timestamp) when the message was sent if the message was replied from another chat; 0 for messages from the same chat
    , content          :: Maybe MessageContent.MessageContent -- ^ Media content of the message if the message was replied from another chat; may be null for messages from the same chat and messages without media. Can be only one of the following types: messageAnimation, messageAudio, messageContact, messageDice, messageDocument, messageGame, messageInvoice, messageLocation, messagePhoto, messagePoll, messagePremiumGiveaway, messageSticker, messageStory, messageText (for link preview), messageVenue, messageVideo, messageVideoNote, or messageVoiceNote
    }
