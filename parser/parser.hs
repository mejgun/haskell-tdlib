...
main = do
  ...
  where
    ...
    removeUnnecessary t =
      T.unlines $
        filter
          ( \x -> let s = T.strip x in s /= T.empty && not (T.isPrefixOf "//" s)
          )
          $ T.lines t
...
writeToFiles ::
  ...
  IO ()
writeToFiles addr modName mp cmnts =
  mapM_
    ( \(a, b) -> do
        writeFile
          (T.unpack (T.concat ["../src/", addr, "/", a, ".hs"]))
          (wr a b)
        writeFile
          (T.unpack (T.concat ["../src/", addr, "/", a, ".hs-boot"]))
          (whr a)
    )
    $ Map.toList mp
  where
    ...
    wgr d e =
      ...
      where
        wwgr = map (\(a, b) -> T.concat [toTitle a, " ", (wwwgr b)]) e
        wwwgr l = T.intercalate " " (map (\(_, b) -> b) l)
    ...
    www e =
      case T.intercalate ", " (map (\(a, b) -> T.concat [a, " :: ", b]) e) of
        "" -> ""
        p -> T.concat [" { ", p, " } "]
    toShowString :: T.Text -> [(T.Text, [(T.Text, T.Text)])] -> T.Text
    toShowString m e =
      T.concat
        [ "\n\ninstance Show ",
          m,
        ...
        [ "\n\ninstance T.ToJSON ",
          ...
          T.concat
            [ "  t <- obj A..: \"@type\" :: T.Parser String\n",
              ...
              T.intercalate "\n" $
                ( map
                    (\(a, _) -> T.concat ["   \"", a, "\" -> parse", toTitle a, " v"])
                    e
                )
                  ++ ["   _ -> mempty"],
              "\n  where\n",
  ...
  where
    ...
    gogo m e =
      ...
      where
        ...
        myInsert mm a b = do
          let key =
                if a `elem` ["type", "data", "id", "length", "error", "filter"]
                  then addUndrscr a
                  else a
...
parseComments l = do
  ...
  let s1 =
        map
          ( \y ->
              filter (\x -> (x /= T.empty) && not (T.isPrefixOf (T.pack "@class ") x)) $
                T.splitOn "\n//" y
          )
          s
  ...
  let m3 =
        Map.map
          ( \t -> foldr (\x acc -> (filter (/= T.empty) (splitAtAt x)) ++ acc) [] t
          )
          m2
  ...
  where
    ...
    splitAtAt t =
      let (x : xs) = T.splitOn "@" t
       in if length xs > 0 then x : map toHyperlinkFirst xs else [t]
